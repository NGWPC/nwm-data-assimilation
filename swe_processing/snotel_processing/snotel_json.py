...
class DataLoader:
    ...
    @staticmethod
    def load_json_file(filepath: str | Path) -> list | None:
    ...
    @staticmethod
    def load_json_directory(directory_path: str | Path) -> dict:
...
class DataParser:
    """Data Parser for SNOTEL data."""

    @staticmethod
    def parse_swe_station(swe_data: list) -> str | None:
    ...
    @staticmethod
    def parse_station_data(data: list) -> pd.DataFrame:
    ...
    @staticmethod
    def parse_swe_data(
        swe_data: list, swe_station: str, local_hour: int, day_offset: int
    ) -> pd.DataFrame:
    ...
    @staticmethod
    def merge_data(station_df: pd.DataFrame, swe_df: pd.DataFrame) -> pd.DataFrame:
    ...
    @staticmethod
    def get_offset(swe_station: str, station_df: pd.DataFrame) -> int | None:
        """Extract the offset from UTC value from the station dataframe, using the station triplet from the swe data json file to index.
        ...
        """
        station_offset = station_df[station_df["triplet"] == swe_station][
            "offset"
        ].iloc[0]
    ...
    @staticmethod
    def get_local_time_for_utc(utc_hour: int, offset: int) -> tuple[int, int]:
...
class CSVWriter:
    """CSV Writer for processed data."""

    @staticmethod
    def write_to_csv(merged_df: pd.DataFrame, output_dir: str | Path) -> str | None:
...
class JSONProcessor:
    """Process JSON files and generate CSV output."""

    def __init__(self, station_filepath=None, swe_filepath=None, output_dir=None):
        """Initialize the JSONProcessor with file paths."""
    ...
    def process(self):
        """Process the JSON files and generate CSV output."""
        t_start = time.time()
        ...
        for file in self.swe_filelist:
            ...
            (self.local_hour, self.day_offset) = DataParser.get_local_time_for_utc(
                self.utc_hour, self.offset
            )
            self.swe_df = DataParser.parse_swe_data(
                self.swe_data, self.swe_station, self.local_hour, self.day_offset
            )
...
def get_options(args_list=None):
    ...
    parser.add_argument(
        "swe_filepath", type=str, help="Path that points to SNOTEL swe data json files."
    )
    parser.add_argument("output_dir", type=str, help="Path to write csv output.")
